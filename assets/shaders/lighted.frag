...
// for more realistic lighting dependeing on normal position
struct Sky {
...
void main()
{
    ...
    for(int light_idx = 0; light_idx < min(MAX_LIGHTS, light_count); light_idx++){
        ...
        if(light.type == DIRECTIONAL){
            ...
            // get attemuation based on distance and clculation method
            attenuation = 1.0 / dot(light.attenuation, vec3(d*d, d, 1.0));
            
            if(light.type == SPOT){
                // angle fro source
                float angle = acos(dot(light.direction, -world_to_light_dir));
                // similar to sigmoid outer angle above it = 0, inner angle below it = 1 and interpolate between them 
