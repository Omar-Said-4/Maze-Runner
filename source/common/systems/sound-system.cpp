...
void SoundSystem::initMap(){
 ...
 void SoundSystem::destroyMenuSounds()
 {
    ...
    void SoundSystem::play_custom_sound(std::string sound_name, bool wait_tell_finish,bool loop){
        if (ma_sound_is_playing(SoundSystem::Audios[sound_name])&& !wait_tell_finish &&  (sound_name.compare("WALK")==0||sound_name.compare("RUN")==0||sound_name.compare("WALL_COLLISION")==0)) {
